def add_time(start, duration, day=None):
    ...
    if new_time_minute // 60 != 0:
        new_time_hour = (
            int(start_time[0]) + int(duration_time[0]) + new_time_minute // 60
        )
        new_time_minute %= 60
    else:
    ...
    if day != None:
        new_day = list(day_dict.keys())[
            list(day_dict.values()).index(
                (day_dict.get(day.lower()) + duration_day) % 7
            )
        ]
        new_time = (
            str(new_time_hour)
        ...
        new_time = (
            str(new_time_hour) + ":" + padding + str(new_time_minute) + " " + am_pm
        )
