class Category:
        ...
        for i in range(len(self.ledger)):
            ...
            if description_length > 23:
                string += (
                    ...
                    + ((30 - 23 - len(str(float_money))) * " ")
                    + str(float_money)
                    + "\n"
                )
            else:
                string += (
                    self.ledger[i]["description"]
                    + ((30 - description_length - len(str(float_money))) * " ")
                    + str(float_money)
                    + "\n"
                )
    ...
    def deposit(self, deposit_money, deposit_description=""):
        self.ledger.append(
            {"amount": deposit_money, "description": deposit_description}
        )
    ...
    def withdraw(self, withdraw_money, withdraw_description=""):
        if self.money < withdraw_money:
            return False
        self.ledger.append(
            {"amount": -withdraw_money, "description": withdraw_description}
        )
...
def create_spend_chart(class_list):
    ...
    for class_name in class_list:
        withdraw_percentage.append(
            [class_name.name, int(class_name.withdraw_total * 10 / withdraw_sum) * 10]
        )
